...
stdin = stdout = stderr = &UART_stdinout;
    ...
    //volatile char *special_output_port = (volatile char *)0x20;
    //volatile char *special_input_port = (volatile char *)0x22;
#endif // defined (_SIMULAVR_)

void UART0::begin(uint32_t baud)
{   
    #if defined (_SIMULAVR_) // compiling for simulavr
        // do nothing
    #elif defined (__AVR_ATmega32U4__) // prep usb cdc
        SetupHardware();
    #elif defined (__AVR_ATmega328P__) // uart init
    ...
    #else // which uC we are compiling for ?
        #error Unknown microcontroller !
    #endif   
	init_UART_stdio();
}

inline int UART0::writeChar(char ch)
{
    #if defined (_SIMULAVR_) // do simulavr
        special_output_port = ch;  // send char to output register
    #elif defined (__AVR_ATmega32U4__) // do usb cdc
        if (RingBuffer_IsFull(&USARTtoUSB_Buffer))
            return -1;
        RingBuffer_Insert(&USARTtoUSB_Buffer, ch);
        usb_cdc_loop();
    #elif defined (__AVR_ATmega328P__) // do normal uart
...
{	
    #if defined (_SIMULAVR_) // do simulavr
        return (char)special_input_port; // return input register
    #elif defined (__AVR_ATmega32U4__) // do usb cdc
        usb_cdc_loop();
